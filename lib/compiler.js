...
marked.setOptions({
    gfm: true,
    langPrefix: 'prettyprint lang-',//linenums
    renderer: renderer
});
 ...
 * @param  {Object} opts 配置对象
 */
exports.compileOtherPage = function (root, opts, pageName) {
    if (root.slice(-1) !== '/') {
        root += '/';
    }

    var tpl = loadTemplate(root, pageName);

    if (typeof opts !== 'object') {
        opts = parser.parseYaml(root);
    }
    opts.filename = root + 'templates/' + pageName + '.ejs';

    var html = ejs.render(tpl, opts);

    fs.writeFileSync(root + '/' + pageName + '.html', html);

    logger.info('Build '+pageName+'.html completed!');
};
 ...
 * @param  {Object} opts 配置对象
 */
exports.compileDataJson = function (root, opts) {
    if (root.slice(-1) !== '/') {
        root += '/';
    }
    ...
    var output = root + 'data.json';
    // 过滤
    opts.site.rss && delete opts.site.rss;
    opts.article && delete opts.article;
    opts.page && delete opts.page;
    opts.coreTags && delete opts.coreTags;
    opts.filename && delete opts.filename;
    opts.tags && delete opts.tags;

    for(var i = 0; i < opts.articles.length; i++){
        opts.articles[i].content && delete opts.articles[i].content;//TODO
    }
    fs.writeFileSync(output, JSON.stringify(opts));
    logger.info('Build DataJson completed!');
};
...
exports.compile = function (root) {
    ...
    this.compileDataJson(root, opts);
    this.compileOtherPage(root, opts, "archive");
    this.compileOtherPage(root, opts, "about");
};
